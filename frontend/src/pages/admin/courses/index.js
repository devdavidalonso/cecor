...
import { HiAcademicCap, HiSearch, HiX, HiOutlineRefresh, HiPlus } from 'react-icons/hi';
...
import CourseCard from '../../../components/courses/CourseCard'; // Assumindo que existe um componente específico para admin
...
export default function AdminCourses() {
  ...
  const fetchCourses = async () => {
    ...
    try {
      ...
      // Garantir que data é um array antes de atualizar o estado
      if (Array.isArray(data)) {
        setCourses(data);
      } else {
        console.error('Dados de cursos inválidos:', data);
        setCourses([]);
        showError('Formato de dados inválido ao carregar cursos');
      }
    } catch (error) {
      handleApiError(error, showError, 'Erro ao carregar cursos');
      setCourses([]); // Garantir que courses é um array mesmo após um erro
    } finally {
      setLoading(false);
    }
  };
  ...
  // Navegar para a página de criação de curso
  const handleCreateCourse = () => {
    router.push('/admin/courses/new');
  };

  // Navegar para a página de detalhes/edição de curso
  const handleEditCourse = (courseId) => {
    router.push(`/admin/courses/${courseId}`);
  };

  // Filtrar cursos com tratamento seguro para array
  const getFilteredCourses = () => {
    // Verificar se courses é um array
    if (!Array.isArray(courses)) {
      console.warn('courses não é um array:', courses);
      return []; // Retornar array vazio se courses não for um array
    }
      ...
      return (
        <Card>
          <div className="text-center py-10">
            ...
            <h3 className="mt-4 text-xl font-medium text-gray-900">Nenhum curso cadastrado</h3>
            <p className="mt-2 text-base text-gray-500">
              Não há cursos cadastrados no sistema. Clique no botão abaixo para criar um novo curso.
            </p>
