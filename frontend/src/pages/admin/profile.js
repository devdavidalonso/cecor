...
export default function AdminProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      setLoading(true);
      try {
        const data = await profileService.getProfile('admin');
        setProfileData(data);
      } catch (error) {
  ...
  const handleProfileUpdate = async (formData) => {
    setLoading(true);
    try {
      const updatedProfile = await profileService.updateProfile(formData, 'admin');
  ...
  const handlePasswordChange = async (passwordData) => {
    ...
    try {
      await profileService.changePassword(passwordData, 'admin');
